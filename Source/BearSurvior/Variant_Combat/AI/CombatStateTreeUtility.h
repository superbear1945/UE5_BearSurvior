...
/**
 *  Instance data struct for the Start Chasing Player StateTree task
 */
USTRUCT()
struct FStateTreeStartChasingPlayerInstanceData
{
	GENERATED_BODY()

	/** AI Controller that will chase the player */
	UPROPERTY(EditAnywhere, Category = Context)
	TObjectPtr<AAIController> Controller;

	/** Player character to chase */
	UPROPERTY(EditAnywhere, Category = Input)
	TObjectPtr<ACharacter> PlayerToChase;

	/** Chasing speed for the character */
	UPROPERTY(EditAnywhere, Category = Parameter, meta = (ClampMin = 0, ClampMax = 2000, Units = "cm/s"))
	float ChaseSpeed = 600.0f;

	/** Previous speed to restore when stop chasing */
	UPROPERTY(VisibleAnywhere)
	float PreviousSpeed = 0.0f;

	/** Whether currently chasing */
	UPROPERTY(VisibleAnywhere)
	bool bIsChasing = false;
};

/**
 *  StateTree task to start chasing the player
 *  When the state is entered, the AI will start chasing the player with increased speed
 *  When the state is exited, the AI will stop chasing and restore the original speed
 */
USTRUCT(meta=(DisplayName="Start Chasing Player", Category="Combat"))
struct FStateTreeStartChasingPlayerTask : public FStateTreeTaskCommonBase
{
	GENERATED_BODY()

	/* Ensure we're using the correct instance data struct */
	using FInstanceDataType = FStateTreeStartChasingPlayerInstanceData;
	virtual const UStruct* GetInstanceDataType() const override { return FInstanceDataType::StaticStruct(); }

	/** Runs when the owning state is entered - starts chasing the player */
	virtual EStateTreeRunStatus EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const override;

	/** Runs while the owning state is active - continues chasing behavior */
	virtual EStateTreeRunStatus Tick(FStateTreeExecutionContext& Context, const float DeltaTime) const override;

	/** Runs when the owning state is ended - stops chasing and restores speed */
	virtual void ExitState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const override;
