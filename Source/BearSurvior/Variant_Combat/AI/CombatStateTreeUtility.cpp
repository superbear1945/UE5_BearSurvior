...
EStateTreeRunStatus FStateTreeStartChasingPlayerTask::EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	// have we transitioned from another state?
	if (Transition.ChangeType == EStateTreeStateChangeType::Changed)
	{
		...
		// get the controlled pawn (the enemy character)
		ACharacter* ControlledCharacter = Cast<ACharacter>(InstanceData.Controller->GetPawn());
		if (!ControlledCharacter)
			return EStateTreeRunStatus::Failed;

		// save the previous speed for restoration later
		InstanceData.PreviousSpeed = ControlledCharacter->GetCharacterMovement()->MaxWalkSpeed;

		// set the chase speed
		ControlledCharacter->GetCharacterMovement()->MaxWalkSpeed = InstanceData.ChaseSpeed;

		// set focus on the player to chase
		if (InstanceData.PlayerToChase)
			InstanceData.Controller->SetFocus(InstanceData.PlayerToChase);

		// mark as chasing
		InstanceData.bIsChasing = true;
	}

	return EStateTreeRunStatus::Running;
}

EStateTreeRunStatus FStateTreeStartChasingPlayerTask::Tick(FStateTreeExecutionContext& Context, const float DeltaTime) const
{
	// get the instance data
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);

	// if we have a valid player to chase, update the focus to the player's current location
	if (InstanceData.PlayerToChase && InstanceData.bIsChasing)
		InstanceData.Controller->SetFocus(InstanceData.PlayerToChase);

	return EStateTreeRunStatus::Running;
}

void FStateTreeStartChasingPlayerTask::ExitState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	// have we transitioned to another state?
	if (Transition.ChangeType == EStateTreeStateChangeType::Changed)
	{
		...
		// mark as not chasing
		InstanceData.bIsChasing = false;
	}
}

#if WITH_EDITOR
FText FStateTreeStartChasingPlayerTask::GetDescription(const FGuid& ID, FStateTreeDataView InstanceDataView, const IStateTreeBindingLookup& BindingLookup, EStateTreeNodeFormatting Formatting /*= EStateTreeNodeFormatting::Text*/) const
{
	return FText::FromString("<b>Start Chasing Player</b>");
}
#endif // WITH_EDITOR