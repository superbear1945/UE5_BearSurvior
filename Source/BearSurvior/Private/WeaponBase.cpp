...
/**
 * 在游戏开始时初始化武器状态。
 * 先让 AItemBase 解析公共物品数据，再初始化 WeaponBase
 * 公共状态并通知攻击组件解析自身数据。
 */
 ...
 * 返回武器是否已损坏。
 */
bool AWeaponBase::IsBroken() const { return CurrentDurability <= 0.0f; }
 ...
 */
void AWeaponBase::PrimaryUseEnd_Implementation() { StopAttack(); }
...
/**
 * 装备武器时将事件转发给当前攻击组件，并按外部传入的插槽名称附着到角色骨骼。
 * 远程组件会在这里同步加载枪声等装备期需要保持硬引用的资源。
 * @param CharacterOwner 装备该武器的角色。
 * @param AttachSocketName 武器需要附着到的角色骨骼插槽名称。
 */
void AWeaponBase::Equip_Implementation(ACharacter *CharacterOwner, FName AttachSocketName) {
  ...
  if (AttachSocketName.IsNone()) {
    UE_LOG(LogTemp, Warning, TEXT("[%s] Equip 失败：附着插槽名称为空"), *GetNameSafe(this));
    return;
  }

  USkeletalMeshComponent *CharacterMesh = CharacterOwner->GetMesh();
  if (!CharacterMesh->DoesSocketExist(AttachSocketName)) {
    UE_LOG(LogTemp, Warning, TEXT("[%s] Equip 失败：角色网格体不存在插槽 [%s]"), *GetNameSafe(this), *AttachSocketName.ToString());
    return;
  }
  ...
  // 转发装备事件给攻击组件，确保攻击组件在武器附着前完成装备期资源缓存。
