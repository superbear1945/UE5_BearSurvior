 ...
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FOnHealthDamagedSignature,
                                              UHealthComponent *,
                                              HealthComponent, float,
                                              DamageAmount, float, NewHP,
                                              AActor *, DamageCauser);
 ...
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FOnHealthHealedSignature,
                                              UHealthComponent *,
                                              HealthComponent, float,
                                              HealAmount, float, NewHP,
                                              AActor *, Healer);
 ...
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnHealthDeathSignature,
                                             UHealthComponent *,
                                             HealthComponent, AActor *,
                                             InstigatorActor);

/**
 * 生命值组件：负责管理宿主 Actor 的生命值、死亡判定与相关事件广播。
 *
 * 该组件聚焦于“数值状态”本身，不直接耦合角色动画、AI
 * 停止、掉落物生成等具体业务表现。 外部系统可以通过监听组件事件来驱动
 * UI、特效、状态切换与销毁流程。
 */
...
protected:
   ...
   *
   * 一旦进入死亡状态，ApplyDamage()、ApplyHealing() 与 Die()
   * 都会按规则做幂等保护。
   */
  UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Health")
  bool bIsDead = false;
...
public:
   ...
   * 当 bScaleCurrentHP 为 true
   * 时，会按旧生命百分比缩放当前生命值；否则仅将当前生命值钳制到新的合法区间。
   ...
   */
  UFUNCTION(BlueprintCallable, Category = "Health")
  virtual void SetMaxHP(float NewMaxHP, bool bScaleCurrentHP,
